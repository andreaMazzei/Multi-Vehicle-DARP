...
all_points, points_no_depots, set1, set2, vehicles, demand, distance, cost, pickup_points, delivery_points, service_time = inp.all_points, inp.points_no_depots, inp.set1, inp.set2, inp.vehicles, inp.demand, inp.distance, inp.cost, inp.pickup_points, inp.delivery_points, inp.service_time
...
# Constraints
model.addConstrs((gp.quicksum(x[i, j, k] for k in vehicles for j in set2 if i != j) == 1 for i in pickup_points)) #ogni client attraversato da un solo veicolo
...
sol = {k: v.X for k, v in x.items() if v.X > 1e-6}
print(sol)
